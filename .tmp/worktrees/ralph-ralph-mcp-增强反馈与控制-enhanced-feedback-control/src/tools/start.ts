import { z } from "zod";
import { parsePrdFile, analyzeStorySize, type StorySizeWarning } from "../utils/prd-parser.js";
...
export interface StartResult {
  ...
  storySizeWarnings?: StorySizeWarning[];
}
...
export async function start(input: StartInput): Promise<StartResult> {
  ...
  // Analyze story size (AC-2, AC-3)
  const sizeAnalysis = analyzeStorySize(prd.userStories);
  ...
  // Add story size warnings if any (AC-4, AC-6: warning only, does not block execution)
  if (sizeAnalysis.hasOversizedStories) {
    result.storySizeWarnings = sizeAnalysis.warnings;
  }
