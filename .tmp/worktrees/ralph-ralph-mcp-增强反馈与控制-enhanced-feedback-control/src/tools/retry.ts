...
export const retryInputSchema = z.object({
  ...
  hint: z.string().optional().describe("User hint to inject into the agent prompt to guide the retry attempt"),
  wipPolicy: z
...
export interface RetryResult {
  ...
  hintInjected?: string; // US-007: The hint that was injected into the agent prompt
}
 ...
 * US-007: Added hint parameter for user guidance injection.
 */
export async function retry(input: RetryInput): Promise<RetryResult> {
  ...
  // US-007: Only allow retry for failed or stopped executions (AC-5)
  // Note: "blocked" stories cause execution status to become "failed"
  if (previousStatus !== "failed" && previousStatus !== "stopped") {
    return {
      success: false,
      branch: input.branch,
      message: `Cannot retry execution with status '${previousStatus}'. Only 'failed' or 'stopped' executions can be retried. (Blocked stories cause execution to become 'failed')`,
      previousStatus,
      agentPrompt: null,
  ...
  // US-007: Include user hint if provided (AC-3)
  const resumeContext = [
  ...
  return {
    success: true,
    branch: input.branch,
    message: `Execution resumed. ${total - completed} stories remaining.${input.hint ? " User hint injected." : ""}`,
    previousStatus,
    agentPrompt,
    progress: {
      completed,
    ...
    worktreeRestored,
    hintInjected: input.hint, // US-007: Include the hint in the result
  };
}
