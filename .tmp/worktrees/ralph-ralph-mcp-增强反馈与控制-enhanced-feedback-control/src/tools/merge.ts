...
export async function merge(input: MergeInput): Promise<MergeResult> {
  ...
  // Generate pre-merge report (US-003)
  let mergeReportPath: string | undefined;
  try {
    // AC-1, AC-2: Get diff statistics
    const diffStats = await getDiffStats(exec.projectRoot, exec.branch);

    // AC-3: Run quality checks for report (even if skipped for merge)
    let qualityChecksResult = { typeCheck: true, build: true };
    if (!input.skipQualityChecks && exec.worktreePath) {
      const qc = await runQualityChecks(exec.worktreePath);
      qualityChecksResult = {
        typeCheck: qc.typeCheck.success,
        build: qc.build.success,
      };
    }

    // AC-4, AC-5: Assess risk
    const riskAssessment = assessRisk(diffStats);

    // Build completed stories data with evidence
    const completedStoriesData = stories
      .filter((s) => s.passes)
      .map((s) => ({
        id: s.storyId,
        title: s.title,
        acCount: s.acceptanceCriteria.length,
        acCompleted: Object.values(s.acEvidence || {}).filter((e) => e.passes).length,
        evidence: Object.entries(s.acEvidence || {})
          .filter(([, e]) => e.passes && e.evidence)
          .map(([, e]) => e.evidence)
          .join("; ")
          .slice(0, 200),
      }));

    // AC-6, AC-7, AC-8: Generate markdown report
    ...
    const reportContent = generateMergeReport(reportData);

    // AC-9: Save report to worktree root
    const reportRoot = exec.worktreePath || exec.projectRoot;
    mergeReportPath = saveMergeReport(reportRoot, exec.branch, reportContent);
    console.log(`>>> Pre-merge report generated: ${mergeReportPath}`);
  } catch (reportError) {
    console.error("Failed to generate pre-merge report:", reportError);
    // Continue with merge even if report generation fails
  }
