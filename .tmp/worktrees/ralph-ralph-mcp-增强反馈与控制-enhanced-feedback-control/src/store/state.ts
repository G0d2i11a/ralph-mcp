...
/**
 * Entry in the stagnation log for tracking warnings and blocks.
 */
export interface StagnationLogEntry {
  timestamp: Date;
  type: "warning" | "blocked";
  reason: "idle_retry" | "no_progress" | "repeated_error";
  message: string;
  details?: {
    similarity?: number; // For idle_retry
    consecutiveCount?: number; // For no_progress or repeated_error
    error?: string; // For repeated_error
  };
}
...
export interface ExecutionRecord {
  ...
  // Enhanced stagnation detection (US-004)
  lastNotes: string | null; // Last notes for similarity comparison
  consecutiveSimilarNotes: number; // Count of consecutive similar notes
  stagnationLog: StagnationLogEntry[]; // Log of stagnation warnings and blocks
  // Agent introspection tracking (US-006)
  lastConfidence: number | null; // Last confidence score from introspection
  consecutiveLowConfidence: number; // Count of consecutive low confidence (<0.5)
...
/**
 * Structured introspection log for Agent self-reflection (US-006).
 * Agents should include this in their notes to provide structured feedback.
 */
export interface IntrospectionLog {
  implemented: string; // Brief summary of what was implemented
  filesChanged: string[]; // List of files that were modified
  learnings: string; // Key learnings from this implementation
  nextSteps: string; // What should be done next
  confidence: number; // 0-1 confidence score for the implementation
}
...
export interface UserStoryRecord {
  ...
  blockedReason?: BlockedReason | null; // Structured reason for blocking
  // Story-level error tracking for auto-blocking
  consecutiveFailures?: number; // Number of consecutive failures
  lastFailureError?: string | null; // Last error for comparison
}
...
/**
 * Serialized stagnation log entry (timestamp as string).
 */
interface SerializedStagnationLogEntry {
  timestamp: string;
  type: "warning" | "blocked";
  reason: "idle_retry" | "no_progress" | "repeated_error";
  message: string;
...
/**
 * Serialized execution record type for JSON storage.
 */
type SerializedExecutionRecord = Omit<ExecutionRecord, "createdAt" | "updatedAt" | "launchAttemptAt" | "mergedAt" | "stagnationLog"> & {
  createdAt: string;
  updatedAt: string;
  launchAttemptAt: string | null;
  mergedAt: string | null;
  stagnationLog: SerializedStagnationLogEntry[];
};

interface StateFileV1 {
  version: 1;
  executions: SerializedExecutionRecord[];
  userStories: UserStoryRecord[];
  mergeQueue: Array<Omit<MergeQueueItem, "createdAt"> & { createdAt: string }>;
  // Archived data for history
  archivedExecutions?: SerializedExecutionRecord[];
...
function deserializeState(file: StateFileV1): StateRuntime {
  const deserializeExecution = (e: StateFileV1["executions"][0]) => ({
    ...
    // Enhanced stagnation detection defaults (US-004)
    lastNotes: typeof (e as any).lastNotes === "string" ? (e as any).lastNotes : null,
    consecutiveSimilarNotes: typeof (e as any).consecutiveSimilarNotes === "number" ? (e as any).consecutiveSimilarNotes : 0,
    stagnationLog: Array.isArray((e as any).stagnationLog)
      ? (e as any).stagnationLog.map((entry: any) => ({
          ...entry,
          timestamp: parseDate(entry.timestamp, "stagnationLog.timestamp"),
        }))
      : [],
  ...
  const deserializeUserStory = (s: UserStoryRecord) => ({
    ...
    acEvidence: typeof (s as any).acEvidence === "object" && (s as any).acEvidence !== null
      ? (s as any).acEvidence
      : {},
    blocked: typeof (s as any).blocked === "boolean" ? (s as any).blocked : false,
    blockedReason: (s as any).blockedReason || null,
    consecutiveFailures: typeof (s as any).consecutiveFailures === "number" ? (s as any).consecutiveFailures : 0,
    lastFailureError: typeof (s as any).lastFailureError === "string" ? (s as any).lastFailureError : null,
  });
...
function serializeState(state: StateRuntime): StateFileV1 {
  const serializeExecution = (e: ExecutionRecord) => ({
    ...
    // Serialize stagnationLog timestamps
    stagnationLog: e.stagnationLog.map((entry) => ({
      ...entry,
 ...
 * Enhanced with US-004 thresholds
 */
export const STAGNATION_THRESHOLDS = {
  ...
  // US-004 Enhanced thresholds
  SIMILAR_NOTES_WARNING: 2, // Warn after 2 consecutive similar notes (>80% similarity)
  NO_FILES_WARNING: 2, // Warn after 2 consecutive filesChanged: 0
  SAME_ERROR_AUTO_BLOCK: 3, // Auto-block after 3 consecutive same errors (Levenshtein < 20%)
  NOTES_SIMILARITY_THRESHOLD: 0.8, // 80% similarity threshold for notes
  ERROR_SIMILARITY_THRESHOLD: 0.8, // 80% similarity (Levenshtein distance < 20%)
};

export type StagnationType = "no_progress" | "repeated_error" | "max_loops" | "idle_retry" | null;

/**
 * Calculate Levenshtein distance between two strings.
 * Used for detecting similar errors and notes (US-004).
 */
export function levenshteinDistance(a: string, b: string): number {
  if (a.length === 0) return b.length;
  if (b.length === 0) return a.length;

  // Truncate very long strings to avoid performance issues
  const maxLen = 1000;
  const strA = a.length > maxLen ? a.slice(0, maxLen) : a;
  const strB = b.length > maxLen ? b.slice(0, maxLen) : b;
 ...
 */
export function calculateSimilarity(a: string | null, b: string | null): number {
...
export interface StagnationCheckResult {
  ...
  metrics: {
    ...
    // US-004 Enhanced metrics
    consecutiveSimilarNotes: number;
    lastNotes: string | null;
    // US-006 Confidence metrics
    lastConfidence: number | null;
    consecutiveLowConfidence: number;
  };
  // US-004: Warnings that don't block but should be surfaced
  warnings: Array<{
...
export async function checkStagnation(executionId: string): Promise<StagnationCheckResult> {
  return readState((s) => {
    ...
    const metrics = {
      ...
      consecutiveSimilarNotes: exec.consecutiveSimilarNotes,
      lastNotes: exec.lastNotes,
      lastConfidence: exec.lastConfidence,
      consecutiveLowConfidence: exec.consecutiveLowConfidence,
    };

    const warnings: StagnationCheckResult["warnings"] = [];

    // US-006: Check for low confidence warning
    let lowConfidenceWarning: StagnationCheckResult["lowConfidenceWarning"] = undefined;
    if (exec.consecutiveLowConfidence >= 2 && exec.lastConfidence !== null) {
      lowConfidenceWarning = {
        consecutiveCount: exec.consecutiveLowConfidence,
        lastConfidence: exec.lastConfidence,
        message: `Low confidence (${(exec.lastConfidence * 100).toFixed(0)}%) detected ${exec.consecutiveLowConfidence} consecutive times - manual intervention recommended`,
        suggestIntervention: true,
      };
    }

    // US-004: Check for idle retry warning (similar notes)
    if (exec.consecutiveSimilarNotes >= STAGNATION_THRESHOLDS.SIMILAR_NOTES_WARNING) {
      warnings.push({
        type: "idle_retry",
        message: `Similar notes detected ${exec.consecutiveSimilarNotes} times - agent may be stuck in idle retry loop`,
        severity: exec.consecutiveSimilarNotes >= 3 ? "critical" : "warning",
      });
    }

    // US-004: Check for no progress warning
    if (exec.consecutiveNoProgress >= STAGNATION_THRESHOLDS.NO_FILES_WARNING &&
        exec.consecutiveNoProgress < STAGNATION_THRESHOLDS.NO_PROGRESS_THRESHOLD) {
      warnings.push({
        type: "no_progress",
        message: `No file changes for ${exec.consecutiveNoProgress} consecutive loops`,
        severity: "warning",
      });
    }

    // US-004: Check for repeated error warning (approaching threshold)
    if (exec.consecutiveErrors >= 2 && exec.consecutiveErrors < STAGNATION_THRESHOLDS.SAME_ERROR_AUTO_BLOCK) {
      warnings.push({
        type: "repeated_error",
        message: `Same error repeated ${exec.consecutiveErrors} times - approaching auto-block threshold`,
        severity: "warning",
      });
    }

    // Check no progress threshold (blocking)
    ...
    // Check repeated error threshold (blocking)
 ...
 * Enhanced with US-004: notes similarity detection and stagnation logging.
 * Enhanced with US-006: confidence tracking and low confidence detection.
 */
export async function recordLoopResult(
  ...
  return mutateState(async (s) => {
    ...
    if (filesChanged === 0) {
      ...
      // US-004: Log warning for consecutive no progress
      if (exec.consecutiveNoProgress === STAGNATION_THRESHOLDS.NO_FILES_WARNING) {
        const logEntry: StagnationLogEntry = {
          timestamp: now,
          type: "warning",
          reason: "no_progress",
          message: `No file changes for ${exec.consecutiveNoProgress} consecutive loops`,
          details: { consecutiveCount: exec.consecutiveNoProgress },
        };
        exec.stagnationLog.push(logEntry);
        warnings.push({
          type: "no_progress",
    ...
    // US-004: Track similar notes (idle retry detection)
    if (notes) {
      const similarity = calculateSimilarity(exec.lastNotes, notes);
      if (similarity >= STAGNATION_THRESHOLDS.NOTES_SIMILARITY_THRESHOLD) {
        exec.consecutiveSimilarNotes++;

        // Log warning for consecutive similar notes
        if (exec.consecutiveSimilarNotes === STAGNATION_THRESHOLDS.SIMILAR_NOTES_WARNING) {
          const logEntry: StagnationLogEntry = {
            timestamp: now,
            type: "warning",
            reason: "idle_retry",
            message: `Similar notes detected ${exec.consecutiveSimilarNotes} times (${Math.round(similarity * 100)}% similarity) - possible idle retry loop`,
            details: { similarity, consecutiveCount: exec.consecutiveSimilarNotes },
          };
          exec.stagnationLog.push(logEntry);
          warnings.push({
            type: "idle_retry",
        ...
        exec.consecutiveSimilarNotes = 0;
      }
      exec.lastNotes = notes;
    }

    // US-004: Track repeated errors with Levenshtein similarity
    if (error) {
      const errorSimilarity = calculateSimilarity(exec.lastError, error);
      if (errorSimilarity >= STAGNATION_THRESHOLDS.ERROR_SIMILARITY_THRESHOLD) {
        exec.consecutiveErrors++;

        // US-004: Auto-block after 3 consecutive same errors (AC-4)
        if (exec.consecutiveErrors === STAGNATION_THRESHOLDS.SAME_ERROR_AUTO_BLOCK) {
          const logEntry: StagnationLogEntry = {
            timestamp: now,
            type: "blocked",
            reason: "repeated_error",
            message: `Auto-blocked: Same error repeated ${exec.consecutiveErrors} times (${Math.round(errorSimilarity * 100)}% similarity)`,
            details: { similarity: errorSimilarity, consecutiveCount: exec.consecutiveErrors, error: error.slice(0, 200) },
          };
          exec.stagnationLog.push(logEntry);
        } else if (exec.consecutiveErrors === 2) {
          // Warning at 2 consecutive errors
          const logEntry: StagnationLogEntry = {
            timestamp: now,
            type: "warning",
            reason: "repeated_error",
    ...
    // US-006: Track low confidence
    let lowConfidenceWarning: StagnationCheckResult["lowConfidenceWarning"] = undefined;
    if (confidence !== undefined && confidence !== null) {
      exec.lastConfidence = confidence;
      if (confidence < 0.5) {
        exec.consecutiveLowConfidence++;
        // Suggest intervention after 2 consecutive low confidence updates
        if (exec.consecutiveLowConfidence >= 2) {
          lowConfidenceWarning = {
            consecutiveCount: exec.consecutiveLowConfidence,
            lastConfidence: confidence,
            message: `Low confidence (${(confidence * 100).toFixed(0)}%) detected ${exec.consecutiveLowConfidence} consecutive times - manual intervention recommended`,
            suggestIntervention: true,
          };
        }
      } else {
        exec.consecutiveLowConfidence = 0;
      }
    }
    ...
    // US-004: Auto-block after 3 consecutive same errors (Levenshtein < 20%)
    if (exec.consecutiveErrors >= STAGNATION_THRESHOLDS.SAME_ERROR_AUTO_BLOCK) {
      exec.status = "failed";
      return {
        isStagnant: true,
        type: "repeated_error" as StagnationType,
        message: `Stagnation detected: Same error repeated ${exec.consecutiveErrors} times (auto-blocked)`,
        metrics,
        warnings,
        lowConfidenceWarning,
      };
    ...
    // Original threshold for repeated errors (still applies)
    if (exec.consecutiveErrors >= STAGNATION_THRESHOLDS.SAME_ERROR_THRESHOLD) {
      exec.status = "failed";
      return {
        isStagnant: true,
 ...
 * Note: stagnationLog is preserved for history tracking.
 */
export async function resetStagnation(executionId: string): Promise<void> {
  return mutateState((s) => {
    ...
    exec.consecutiveErrors = 0;
    exec.lastError = null;
    // US-004: Reset enhanced stagnation fields
    exec.consecutiveSimilarNotes = 0;
    exec.lastNotes = null;
    // US-006: Reset confidence tracking
    exec.consecutiveLowConfidence = 0;
    // Note: stagnationLog is NOT reset - it's preserved for history
    exec.updatedAt = new Date();
  });
}
