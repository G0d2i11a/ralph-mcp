  ...
  return {
    tools: [
      ...
      },
      {
        name: "ralph_retry",
        description:
          "Retry a failed/stopped PRD execution. Resets stagnation counters, handles uncommitted changes (WIP), and generates a new agent prompt to continue from where it left off. Use the hint parameter to provide guidance to the agent.",
        annotations: {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            hint: {
              type: "string",
              description: "User hint to inject into the agent prompt to guide the retry attempt (e.g., 'Try using a different API endpoint' or 'The DATABASE_URL is now configured')",
            },
            wipPolicy: {
              type: "string",
