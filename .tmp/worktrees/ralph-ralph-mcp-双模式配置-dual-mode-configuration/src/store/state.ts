...
import { type ExecutionMode, DEFAULT_MODE, isValidMode, getModeConfig, getModeBadge } from "../config/modes.js";
...
export interface ExecutionRecord {
  ...
  mode: ExecutionMode; // Execution mode (exploration, delivery, hotfix)
...
export type AcStatus = "passed" | "failed" | "untested" | "partial";

export interface AcEvidence {
  passes: boolean;
  status?: AcStatus; // For soft AC in exploration mode
  evidence?: string;
  command?: string;
  output?: string;
  blockedReason?: string;
  nextSteps?: string; // Required for untested/partial AC in exploration mode
}
...
function deserializeState(file: StateFileV1): StateRuntime {
  const deserializeExecution = (e: StateFileV1["executions"][0]) => ({
    ...
    // Mode defaults for backward compatibility (defaults to 'delivery')
    mode: (typeof (e as any).mode === "string" && isValidMode((e as any).mode)) ? (e as any).mode : DEFAULT_MODE,
...
export const STAGNATION_THRESHOLDS = {
  ...
  MAX_LOOPS_PER_STORY: 10, // Safety limit per story (default, overridden by mode)
};

/**
 * Get mode-specific stagnation thresholds.
 * Uses maxRetries from mode config to scale thresholds.
 */
export function getModeStagnationThresholds(mode: ExecutionMode): {
  noProgressThreshold: number;
  sameErrorThreshold: number;
  maxLoopsPerStory: number;
} {
  const modeConfig = getModeConfig(mode);
  // Scale thresholds based on maxRetries
  // exploration (7 retries): more lenient
  // delivery (3 retries): default
  // hotfix (2 retries): stricter
  const scale = modeConfig.maxRetries / 3; // 3 is the delivery baseline

  return {
    noProgressThreshold: Math.max(2, Math.round(STAGNATION_THRESHOLDS.NO_PROGRESS_THRESHOLD * scale)),
    sameErrorThreshold: Math.max(3, Math.round(STAGNATION_THRESHOLDS.SAME_ERROR_THRESHOLD * scale)),
    maxLoopsPerStory: Math.max(5, Math.round(STAGNATION_THRESHOLDS.MAX_LOOPS_PER_STORY * scale)),
  };
}
...
export async function checkStagnation(executionId: string): Promise<StagnationCheckResult> {
  return readState((s) => {
    ...
    const thresholds = getModeStagnationThresholds(exec.mode);
    const modeBadge = getModeBadge(exec.mode);
    ...
    if (exec.consecutiveNoProgress >= thresholds.noProgressThreshold) {
      return {
        isStagnant: true,
        type: "no_progress",
        message: `${modeBadge} No file changes for ${exec.consecutiveNoProgress} consecutive loops (threshold: ${thresholds.noProgressThreshold})`,
        metrics,
      };
    }

    // Check repeated error threshold
    if (exec.consecutiveErrors >= thresholds.sameErrorThreshold) {
      return {
        isStagnant: true,
        type: "repeated_error",
        message: `${modeBadge} Same error repeated ${exec.consecutiveErrors} times (threshold: ${thresholds.sameErrorThreshold}): ${exec.lastError?.slice(0, 100)}`,
        metrics,
      };
    }
    ...
    if (pendingStories.length > 0 && exec.loopCount >= thresholds.maxLoopsPerStory * pendingStories.length) {
      return {
        isStagnant: true,
        type: "max_loops",
        message: `${modeBadge} Exceeded max loops (${exec.loopCount}) for ${pendingStories.length} pending stories (limit: ${thresholds.maxLoopsPerStory} per story)`,
        metrics,
      };
    }
  ...
  return mutateState(async (s) => {
    ...
    if (exec.consecutiveNoProgress >= thresholds.noProgressThreshold) {
      ...
      return {
        ...
        message: `${modeBadge} Stagnation detected: No file changes for ${exec.consecutiveNoProgress} consecutive loops (threshold: ${thresholds.noProgressThreshold})`,
        metrics,
      };
    }
    ...
    if (exec.consecutiveErrors >= thresholds.sameErrorThreshold) {
      ...
      return {
        ...
        message: `${modeBadge} Stagnation detected: Same error repeated ${exec.consecutiveErrors} times (threshold: ${thresholds.sameErrorThreshold})`,
        metrics,
      };
    }
