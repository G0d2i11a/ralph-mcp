...
import { type ExecutionMode, getModeBadge } from "../config/modes.js";
...
export interface ExecutionStatus {
  ...
  modeBadge: string; // Mode badge for display (e.g., "[DELIVERY]")
  progress: string;
...
export async function status(input: StatusInput): Promise<StatusResult> {
  ...
  for (const exec of filtered) {
    ...
    executionStatuses.push({
      ...
      status: exec.status,
      mode: exec.mode,
      modeBadge: getModeBadge(exec.mode),
