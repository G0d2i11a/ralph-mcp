...
import { type ExecutionMode, getModeConfig, getModeBadge, getModeDescription } from "../config/modes.js";
import { getScopeGuardrailsPromptSection } from "./scope-guardrails.js";
import { getEvidenceRequirementsPromptSection } from "./evidence-validation.js";
...
`;
  ...
  const scopeGuardrails = getScopeGuardrailsPromptSection(mode);
  const evidenceRequirements = getEvidenceRequirementsPromptSection(mode);

  // Build mode-specific instructions
  const modeInstructions = getModeInstructions(mode, modeConfig);
...
/**
 * Get mode-specific instructions for the agent prompt.
 */
function getModeInstructions(mode: ExecutionMode, config: import("../config/modes.js").ModeConfig): string {
  switch (mode) {
    case "exploration":
      return `
**Exploration Mode**: You are in exploration mode. This allows:
- Soft AC (untested/partial) with blockedReason and nextSteps
- Higher retry limit (${config.maxRetries} retries)
...
Include \`blockedReason\` and \`nextSteps\` for any untested/partial AC.
`;
    case "delivery":
      return `
